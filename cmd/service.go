...
var serviceCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if flags, _ := cmd.Flags().GetBool("crud"); flags {
			...
			services.TemplateServiceCrud(args[0])
		} else {
			services.TemplateServiceDefault(args[0])
		}
		cmd.Println("Done service creation")
	},
}

func init() {
	serviceCmd.Flags().BoolP("crud", "c", false, "create a template to service")
